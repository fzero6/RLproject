...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		/*
		const bool objectCollision = (strcmp(contacts->contact(i).collision1().c_str(),COLLISION_ITEM) == 0);
		...
		*/
        if((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0))
		{
            ...
            {
                // add reward feature based on episode length to keep robot from stopping
                //float rewardTime = (maxEpisodeLength - episodeFrames);
                rewardHistory = REWARD_WIN * 10.0f; // + rewardTime
    			newReward = true;
    			endEpisode = true;
    			//printf("DEBUG 1:\n");
            }
    		...
    		{
                // issue large penalty for touching arm instead of gripper
                rewardHistory = REWARD_LOSS * 100.0f; //20
    			newReward = true;
    			endEpisode = true;
    			//printf("DEBUG 2:\n");
    		}
...
/*
    ...
    if(()strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0))
    {
      if((strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0))
      {
        rewardHistory = REWARD_WIN * 20.0f;
        newReward = true;
        endEpisode = true;
        printf("DEBUG 1:\n");
      }
    ...
    if(gripBBox.min.z < groundContact)
    {
      checkGroundContact = true;
		...
		if(checkGroundContact)
		{
            ...
            // give large penalty for touching the ground
			rewardHistory = REWARD_LOSS * 100.0f; //10
			newReward     = true;
			endEpisode    = true;
			//printf("DEBUG 4:\n");
		}
		...
		*/
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
        		...
        		// set constant value for calculating smoothed average
        		const float alpha = 0.2f;

				// compute the smoothed moving average of the delta of the distance to the goal
				avgGoalDelta  = (avgGoalDelta * alpha) + distDelta * (1 - alpha);

				// set an intermediary reward when the arm is not touching the ground
				// if arm is moving closer to goal set reward based on distance to goal
				if(avgGoalDelta > 0) //avgGoalDelta
				{
                    // as the distGoal gets smaller, increase the reward
                    rewardHistory = REWARD_WIN + 1/distGoal;
					//printf("DEBUG 5:\n");
				}
				else
				{
					// issue a reward for moving away from the target.
					rewardHistory = REWARD_LOSS;
					//printf("DEBUG 6:\n");
				}
